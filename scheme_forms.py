...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        #next_value = scheme_eval(value.first, env)
        ...
        # BEGIN PROBLEM 10
        Pair_infunc = (Pair(signature.rest, expressions.rest))
        env.define(signature.first, do_lambda_form((Pair_infunc), env))
...
def do_and_form(expressions, env):
    ...
    while expressions:
          if expressions.rest is nil:
             tail = True
          check_num = scheme_eval(expressions.first ,env,tail)
          if is_scheme_false(check_num):
             return check_num
...
def do_mu_form(expressions, env):
    ...
    validate_formals(formals)
    # BEGIN PROBLEM 11
    #1. Creating procedures using lambda and mu expressions
    return MuProcedure(formals, expressions.rest)
    #2. Defining named procedures using define expressions, and
    #3. Calling user-defined procedures.
