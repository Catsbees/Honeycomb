...
def pingpong(n):
    ...
    True
    """
    def inverse_func(end,value,inverse):
        if end==n:
           return value
        if end%8 == 0 or num_eights(end):
           return inverse_func(end+1,value-inverse,inverse*-1)
        else:
           return inverse_func(end+1,value+inverse,inverse)
    return inverse_func(1,1,1)
...
def count_coins(change):
    ...
    def counter(change, coin):
        ...
        if change == 0:
           return 1
        return counter(change,next_smaller_coin(coin)) + counter(change-coin,coin)
