...
def is_always_roll(strategy, goal=GOAL):
    ...
    score1 = 0
    bool = True
    possi= strategy(score0,score1)
    for score0 in range(goal):
        for score1 in range(goal):
              if strategy(score0,score1) == possi:
                 bool = True
              else:
                 return False
    return bool
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    index = 0
    while n <= 10:
            high_dice = make_averaged(roll_dice, total_samples) #call make_averaged func
            roll = high_dice(n, dice) #call averaged_dice func e.g 3
            if roll > counter_dice: #3 > 0 roll >counter_dice
               counter_dice = roll #counter_dice = 3
               index = n  #index = 1
            n +=1 # n = 2 do the loop again.
    return index
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    score_strategy = square_update(0, score, opponent_score, dice=six_sided) #
    if score_strategy - score >= threshold:
        return 0
    else:
       return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
